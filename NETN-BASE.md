...
The NETN-BASE FOM module defines the `HLAobjectRoot` attribute `FederateApplication`, which allows the association of an object with a federate application based on pre-agreed UUID. For example, federation agreements can include a list of all participating federate applications with their assigned UUID. The `FederateApplication` attribute references the federate application with the primary responsibility for the object. Usually, the federate registering the object instance has the primary responsibility. However, in some cases, some other federate may register the object instance and then divest ownership for some or all attributes. In these cases, the `FederateApplication` attribute can trigger the referenced federate application to take attribute ownership and assume its modelling responsibility. 
 
Similarly, the NETN-BASE defines the optional `HLAinteractionRoot` parameter `FederateApplication`. The primary purpose of this parameter is to tag all interactions with sender information. This approach allows better logging of interactions for analysis and debugging. 
...
The NETN-BASE FOM module defines the optional `HLAobjectRoot` attribute `Time` for timestamping objects with the creation time specified as scenario time. Similarly, the NETN-BASE defines the optional `HLAinteractionRoot` parameter `Time`, which represents the send-time of the interaction specified as scenario time.
...
|FederateApplication|UUID|Optional. Reference to the federate application with the main responsibility for modelling the object behaviour.|
